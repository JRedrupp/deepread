...
- [ ] **Pre-existing re-rendered articles (rendered before the sync watermark shipped) stay stale forever.** `SyncService.syncArticles` now redownloads an article when the remote `rendered_at` is newer than the locally-stored one — but a pre-migration row (already downloaded, no locally-stored `rendered_at` yet) has no reliable "did this change" signal, so it's intentionally backfilled with the current remote value *without* redownloading (`ArticleSyncAction.backfillRenderedAt`) rather than treated as "newer." This means the exact incident that motivated the watermark work in the first place — an article re-rendered by a packaging/template fix before this shipped — stays stale on existing installs; only re-renders that happen *after* a device's first post-upgrade sync are picked up. Fixing the backlog would mean one-time forcing a redownload instead of a backfill the first time a `renderedAt == null` row is seen, at the cost of a bulk redownload on upgrade.
