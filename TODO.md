...
- [ ] **Migrate iOS off CocoaPods to Swift Package Manager.** `mobile/pubspec.yaml` sets `flutter: config: enable-swift-package-manager: false` because `workmanager_apple` 0.10.7 made every plugin SPM-capable, which flips Flutter off the checked-in CocoaPods Podfile/`[CP] Check Pods Manifest.lock` build phase mid-build and fails `mobile-ios-compile-check` ("The sandbox is not in sync with the Podfile.lock") — CocoaPods is pinned back on as the only path ever actually verified. The real migration (`pod deintegrate`, delete `ios/Podfile`, validate in Xcode) needs a Mac, which nobody doing this work has access to — same gap as TECH_DEBT.md's "iOS background sync … unverified on real hardware" note.
